//
// Sun.cc -- class used to compute sunrise and sunset times for a given day
//
// derived from sunriset.c, (c) 1989, 1992 by Paul Schlyter, placed in the
// public domain 1992
//
...
//
// the methods as defined in Sun.h are implemented below
//
Sun::Sun(double lon, double lat, double ele) {
	longitude = lon;
	latitude = lat;
	elevation = ele;
}
...
void	Sun::compute(time_t when) {
	struct tm	*lt = localtime(&when);
	if ((lt->tm_year != lastyear) || (lt->tm_mon != lastmonth) ||
		(lt->tm_mday != lastday)) {
		lastyear = lt->tm_year;
		lastmonth = lt->tm_mon;
		lastday = lt->tm_mday;
		__sunriset__(lastyear + 1900, lastmonth + 1, lastday,
			longitude, latitude, elevation, 1, &rise, &set);
	}
}
